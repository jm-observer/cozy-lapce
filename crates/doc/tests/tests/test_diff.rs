...
pub fn _test_screen() -> Result<()> {
    ...
    assert!(visual_lines[3].is_diff() && visual_lines[9].is_diff() && visual_lines[17].is_diff() && !visual_lines[10].is_diff());
    assert!(visual_lines[18].is_diff_delete() && visual_lines[19].is_diff_delete() && !visual_lines[20].is_diff_delete());
    let screen_lines = left_lines
        ._compute_screen_lines(Rect::from_origin_size(
            (0.0, 60.0),
            Size::new(1000., 800.),
        ...
        .0;
    let visual_lines = &screen_lines.visual_lines;
    assert!(visual_lines[0].is_diff() && visual_lines[6].is_diff() && visual_lines[14].is_diff() && !visual_lines[7].is_diff());
    assert!(visual_lines[15].is_diff_delete() && visual_lines[16].is_diff_delete() && !visual_lines[17].is_diff_delete());
