...
// }
// pub fn resolve_delta_rs(rope: &Rope, delta: &RopeDelta) ->
// Result<OriginLinesDelta> {     let delta_compute =
// resolve_delta_compute(delta).unwrap();     debug!("{delta_compute:?}");
...
//                 line_start += 1;
//             }
//             let line_offset = Offset::new(copy_line_start_info.0,
// line_start);             copy_line_start = CopyDelta::Copy {
...
//         // let copy_line_start_info =
// resolve_line_complete_by_start_offset(rope,         //
// offset_delta_compute.copy_end.start)?;         let (line, offset_of_line) = {
//             //
// 为什么line+1，因为无法判断该行是否被影响（也许是在行首加字符），
// // 因此索性不要这行             let line =
// rope.line_of_offset(offset_delta_compute.copy_end.start);             let
// line_offset = rope.offset_of_line(line + 1)?;             (line + 1,
// line_offset)         };
//         let copy_line_end_info = resolve_line_complete_by_end_offset(
//             rope,
...
//             };
//             offset_end += offset_of_line -
// offset_delta_compute.copy_end.start;             copy_line_end =
// CopyDelta::Copy {                 recompute_first_or_last_line:
// recompute_last_line,                 offset: Offset::new(offset_of_line,
// offset_end),                 copy_line,
...
//                 },
//                 (DeltaElement::Copy(start, end),
// DeltaElement::Insert(val_end)) => {                     rs.copy_start =
// Interval::new(*start, *end);                     rs.internal_len =
// val_end.len();                 },
//                 (
//                     DeltaElement::Insert(val),
//                     DeltaElement::Copy(start_end, end_end),
