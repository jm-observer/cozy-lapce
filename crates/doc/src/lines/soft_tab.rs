...
/// The direction to snap. Left is used when moving left, Right when moving
/// right. Nearest is used for mouse selection.
...
/// nearest, left or right edge. This version takes an offset and returns an
/// offset.
...
/// If the cursor is inside a soft tab at the start of the line, snap it to the
/// nearest, left or right edge. This version takes a line/column and returns a
/// column.
...
/// either an column or offset within the line since it is only modified when it
/// makes no difference which is used (since they're equal for spaces).
/// It returns the column or offset within the line (depending on what you
/// passed in).
...
mod tests {
    ...
    fn test_snap_to_soft_tab_line_col() {
        ...
        // Input line, column, and output column for Left, Nearest and Right
        // respectively.
        let test_cases = [
            (0, 0, 0, 0, 0),
            (0, 1, 0, 0, 4),
