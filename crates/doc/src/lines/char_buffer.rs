...
///
/// That is, this structure stores the result of converting [`char`] to
/// [`prim@str`] (which can be accessed as [`&str`]).
///
...
/// or their borrowed or native forms to [`CharBuffer`] fails.
///
/// This `structure` is created by various `CharBuffer::try_from` methods (for
/// example, by the [`CharBuffer::try_from<&str>`] method).
///
...
impl From<CharBuffer> for char {
    ...
    /// # Note
    ///
    /// In general, it is not recommended to perform `char -> CharBuffer ->
    /// char` type conversions, as this may affect performance.
    ///
    /// # Example
    ///
    /// ```
...
impl Deref for CharBuffer {
    ...
    fn deref(&self) -> &Self::Target {
        // SAFETY:
        // - This is the same buffer that we passed to `encode_utf8` during creating
        //   this structure, so valid utf8 is stored there;
        // - The length was directly calculated from the `&str` returned by the
