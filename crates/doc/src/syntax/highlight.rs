...
use crate::language::{LapceLanguage, load_grammar, read_grammar_query};
...
pub fn get_highlight_config(
    ...
    HIGHLIGHT_CONFIGS.with(|configs| {
        ...
        let config = configs.entry(lang).or_insert_with(|| {
            new_highlight_config(
                get_grammar(
                    &lang.grammar_name(),
...
/// Indicates which highlight should be applied to a region of source
/// code.
...
/// Contains the data needed to highlight code written in a particular
/// language.
///
...
impl HighlightConfiguration {
    /// Creates a `HighlightConfiguration` for a given `Language` and
    /// set of highlighting queries.
    ///
    /// # Parameters
    ///
    /// * `language`  - The Tree-sitter `Language` that should be used for
    ///   parsing.
    /// * `highlights_query` - A string containing tree patterns for syntax
    ///   highlighting. This should be non-empty, otherwise no syntax highlights
    ///   will be added.
    /// * `injections_query` -  A string containing tree patterns for injecting
    ///   other languages into the document. This can be empty if no injections
    ///   are desired.
    /// * `locals_query` - A string containing tree patterns for tracking local
    ///   variable definitions and references. This can be empty if local
    ///   variable tracking is not needed.
    ///
    /// Returns a `HighlightConfiguration` that can then be used with
    ...
    pub fn new(
        ...
        // Construct a single query by concatenating the three query
        // strings, but record the range of pattern indices
        // that belong to each individual string.
        ...
        // Find all of the highlighting patterns that are disabled for
        // nodes that have been identified as local variables.
    ...
    ///
    /// Tree-sitter syntax-highlighting queries specify highlights in
    /// the form of dot-separated highlight names like
    /// `punctuation.bracket` and `function.method.builtin`. Consumers
    /// of these queries can choose to recognize highlights with
    /// different levels of specificity. For example, the string
    /// `function.builtin` will match against
    /// `function.builtin.constructor` but will not match
    /// `function.method.builtin` and `function.method`.
    ///
    /// When highlighting, results are returned as `Highlight` values,
    /// which contain the index of the matched highlight this list
    /// of highlight names.
    ...
    pub(crate) fn injection_for_match<'a>(
        ...
        for prop in query.property_settings(query_match.pattern_index) {
            match prop.key.as_ref() {
                // In addition to specifying the language name via the
                // text of a captured node, it can
                // also be hard-coded via a `#set!` predicate
                ...
                // By default, injections do not include the
                // *children* of an `injection.
                // content` node - only the ranges that belong to the
                // node itself. This can be changed using a `#set!`
                // predicate that sets the `injection.
                // include-children` key.
                "injection.include-children" => {
                    included_children = IncludedChildren::All
                },

                // Some queries might only exclude named children but
                // include unnamed children in their
                // `injection.content` node. This can be enabled using
                // a `#set!` predicate that sets the
                // `injection.include-unnamed-children` key.
...
impl<'a> Iterator for HighlightIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        'main: loop {
            // If we've already determined the next highlight
            // boundary, just return it.
            if let Some(e) = self.next_event.take() {
                return Some(Ok(e));
            }

            // Periodically check for cancellation, returning
            // `Cancelled` error if the cancellation flag
            // was flipped.
            ...
            // If none of the layers have any more highlight
            ...
            // Get the next capture from whichever layer has the
            // earliest highlight boundary.
            let range;
            ...
            if let Some((next_match, capture_index)) = captures.peek() {
                ...
                // If any previous highlight ends before this node
                // starts, then before processing this
                // capture, emit the source code up until the end of
                // the previous highlight, and an end
                // event for that highlight.
            ...
            // If there are no more captures, then emit any remaining
            // highlight end events. And if there are none
            // of those, then just advance to the end of the document.
            ...
            // Remove from the local scope stack any local scopes that
            // have already ended.
            while range.start > layer.scope_stack.last().unwrap().range.end {
                layer.scope_stack.pop();
            }

            // If this capture is for tracking local variables, then
            // process the local variable info.
            let mut reference_highlight = None;
            let mut definition_highlight = None;
            while match_.pattern_index < layer.config.highlights_pattern_index {
                // If the node represents a local scope, push a new
                // local scope onto the scope stack.
                ...
                // If the node represents a definition, add a new
                // definition to the local scope at
                ...
                // If the node represents a reference, then try to
                ...
                // Continue processing any additional matches for the
                // same node.
            ...
            // If this exact range has already been highlighted by an
            // earlier pattern, or by a different layer,
            // then skip over this one.
            ...
            // If the current node was found to be a local variable,
            // then skip over any highlighting patterns
            // that are disabled for local variables.
            ...
            // Once a highlighting pattern is found for the current
            // node, skip over any later highlighting
            // patterns that also match this node. Captures
            // for a given node are ordered by pattern index, so these
            // subsequent captures are guaranteed to be
            // for highlighting, not injections or
            // local variables.
            ...
            // If this node represents a local definition, then store
            // the current highlight value on the local
            // scope entry representing this node.
            if let Some(definition_highlight) = definition_highlight {
                *definition_highlight = current_highlight;
            }

            // Emit a scope start event and push the node's end
...
// Compute the ranges that should be included when parsing an
// injection. This takes into account three things:
// * `parent_ranges` - The ranges must all fall within the *current* layer's
//   ranges.
// * `nodes` - Every injection takes place within a set of nodes. The injection
//   ranges are the ranges of those nodes.
// * `includes_children` - For some injections, the content nodes' children
//   should be excluded from the nested document, so that only the content
//   nodes' *own* content is reparsed. For other injections, the content nodes'
//   entire ranges should be reparsed, including the ranges of their children.
