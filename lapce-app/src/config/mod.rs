...
impl LapceConfig {
    ...
    fn merge_config(
        ...
        if let Some(theme) = color_theme_config {
            // TODO: use different color theme basis if the theme declares its color
    ...
    /// # Panics
    /// If the color was not able to be found in either theme, which may be
    /// indicative that it is misspelled or needs to be added to the
    /// base-theme.
    ...
    /// Get the dropdown information for the specific setting, used for the
    /// settings UI. This should aim to efficiently return the data, because
    /// it is used to determine whether to update the dropdown items.
    pub fn get_dropdown_info(&self, kind: &str, key: &str) -> Option<DropdownInfo> {
        match (kind, key) {
            ...
            ("editor", "wrap-style") => Some(DropdownInfo {
                ...
                // kebab-case when displayed in settings, but we
                // would need to map back from the dropdown's value
                // or index.
                active_index: self
                    .wrap_style_list
                    .iter()
    ...
    /// This should be called whenever the configuration is changed, so that it
    /// is persisted.
    pub fn update_file(
        parent: &str,
        key: &str,
        ...
        // terminal.default-profile.{OS} It would be better to not need a
        // special hack.
