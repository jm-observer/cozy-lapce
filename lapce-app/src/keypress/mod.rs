...
impl KeyPressData {
    ...
    pub fn handle_keymatch<T: KeyPressFocus + ?Sized>(
        ...
        match &keymatch {
            KeymapMatch::Full(command) => {
                log::debug!("handle_keymatch {:?}", command);
                self.pending_keypress
                    .update(|(pending_keypress, last_time)| {
                        last_time.take();
                        pending_keypress.clear();
            ...
            KeymapMatch::Prefix => {
                // Here pending_keypress contains only a prefix of some keymap, so
                // let's keep collecting key presses.
                return KeyPressHandle {
                    handled: true,
                    keymatch,
                    keypress,
    ...
    fn get_keymaps(
        ...
        if let Err(err) = loader.load_from_str(DEFAULT_KEYMAPS_COMMON, is_modal) {
            trace!("Failed to load common defaults: {err}");
        }
    ...
    pub fn file(config_directory: &Path) -> Option<PathBuf> {
        LapceConfig::keymaps_file(config_directory)
    }

    fn get_file_array(config_directory: &Path) -> Option<toml_edit::ArrayOfTables> {
        let path = Self::file(config_directory)?;
    ...
    pub fn update_file(
        ...
        let path = Self::file(&common.directory.config_directory)?;
