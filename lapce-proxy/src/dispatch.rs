    ...
    async fn handle_request(
        ...
        match rpc {
            ...
            GlobalSearch {
                ...
                // Perform the search on another thread to avoid blocking the proxy
                // thread
                thread::spawn(move || {
            ...
            DuplicatePath {
                ...
                // we want for when a user duplicates a document.
            ...
            RenamePath { from, to } => {
                // We first check if the destination already exists, because rename
                // can overwrite it and that's not the default
                // behavior we want for when a user renames a document.
                ...
                let result = result
                    .map(|_| {
                        ...
                        if is_dir {
                            // Update all buffers in which a file the renamed
                            // directory is an
                            // ancestor of is open to use the file's new path.
                            // This could be written more nicely if
                            // `HashMap::extract_if` were
                            // stable.
                            ...
                            // If the renamed file is open in a buffer, update it to
                            // use the new path.
            ...
            TestCreateAtPath { path } => {
                // This performs a best effort test to see if an attempt to create an
                // item at `path` or rename an item to `path` will
                // succeed. Currently the only conditions that are
                // tested are that `path` doesn't already
                // exist and that `path` doesn't have a parent that exists and is not
                // a directory.
...
fn git_get_remote_file_url(workspace_path: &Path, file: &Path) -> Result<String> {
    ...
    // Get namespace (e.g. organisation/project in case of GitHub,
    // org/team/team/team/../project on GitLab)
            ...
            if let Err(err) = searcher.search_path(
                ...
                UTF8(|lnum, line| {
                    ...
                    let line = if line.len() > 200 {
                        ...
                        // Note that the start/end are column based, not absolute
